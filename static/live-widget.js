(() => {
  ...
  function bars(id,rows,colors){const n=$(id),t=rows.reduce((a,x)=>a+x[1],0)||1;if(n)n.innerHTML=rows.map(([l,v],i)=>`<div class="map-insight-row"><span>${escape(l)}</span><i style="--share:${Math.round(v/t*100)}%;--chart-color:${colors[i]}"></i><b>${v}</b></div>`).join('')}
  function insights(mapId,bots){bars('map-channel-chart',[...new Set(snapshot.map(x=>+x.channel||1))].sort().map(c=>[`CH${c}`,bots.filter(x=>(+x.channel||1)===c).length]),['#ff8c00','#800080','#30b6ff']);bars('map-empire-chart',[[1,'Shinsoo'],[2,'Chunjo'],[3,'Jinno']].map(([e,l])=>[l,bots.filter(x=>+x.empire===e).length]),['#d95a54','#e8b93f','#4f86d9']);const m=rI.maps||{},g=rI.global||{delay:{mob:100,boss:100},count:{mob:100,boss:100}},n=$('map-respawn-summary'),v=(m.values||{}),st=(m.stones||{});if(n)n.innerHTML=`<div><b>⚔ Potwory</b><small>${v[mapId]?`Nadpisanie mapy · ${v[mapId]} s`:`Globalnie · ${g.delay.mob}% czasu podstawowego`}</small></div><div><b>🗿 Metiny i bossy</b><small>${st[mapId]?`Nadpisanie mapy · ${st[mapId]} s`:`Globalnie · ${g.delay.boss}% czasu podstawowego`}</small></div><div><b>✦ Liczebność</b><small>Potwory ${g.count.mob}% · Metiny/bossy ${g.count.boss}%</small></div>`}
